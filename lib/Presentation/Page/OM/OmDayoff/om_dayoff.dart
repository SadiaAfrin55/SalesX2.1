...
class OmDayoff extends StatefulWidget {
  const OmDayoff({Key? key}) : super(key: key);

  @override
  _OmDayoffState createState() => _OmDayoffState();
}

class _OmDayoffState extends State<OmDayoff> {
...
// 2. compress file and get file.
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      color: Colors.white,
      child: SafeArea(
          child: BlocListener<VmAttendenceCubit, VmAttendenceState>(
              listener: (context, state) {
                if (state is GiveVmSignOffAttendance) {
                  setState(() {
                    circular = false;
                  ...
                  BlocListener<VmAttendenceCubit, VmAttendenceState>(
                    listener: (context, state) {
                      if (state is CheckVmSignOffAtendance) {
                        setState(() {
                          checkSign = (state as CheckVmSignOffAtendance).attendance;
                        });
                      }
                    },
                    child: checkSign == "load" ? Center(
                        child: CircularProgressIndicator()) :
                    checkSign =="no" ? Material(
                      child: Container(
                                              ...
                                              onTap: (){
                                                ...
                                                if (_image == null) {
                                                  ...
                                                  if (role == "RE") {
                                                    ...
                                                    BlocProvider.of<VmAttendenceCubit>(context).giveSignOffAttendance(_image!,useId!,note.text==""?"N/A":note.text,latx!,lonx!,userAddress!);
                                                  }
                                                }
                                              },
                                            ) :
                                    ...
                                    Container(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          const Text('You are done for today',style: TextStyle(
                                              fontSize: 14,
                                              fontWeight: FontWeight.w400,
                                              color: Colors.black
                                          ),),
